...
async function otherEvents(){
    try {
        ...
        let date = data.date
        var eventsP = events.filter(x => x.date < date)
    ...
    eventsP.forEach(createCard)

    const categories = eventsP.reduce((categories, event) => Array.from(new Set([...categories,event.category])), [])
    ...
    function filterCheckCards(){
        let filteredChecks = checkEvents(eventsP)
